...
select
  *,
  attrs -> 'metadata' ->> 'supabase_uuid' as supabase_uuid
from
  stripe.customers;
...
Add metadata to Stripe Product:
user_claim: premium

// TODO script that auto-creates the stripe Premium product & price at 4.99/mo
...
Go to the Auth templates page in your dashboard. In each email template, change {{ .SiteURL }}/auth/confirm to {{ .SiteURL }}/api/auth/confirm.



MUST update reset password this goes to settings page if redirectTo is basePath ->
